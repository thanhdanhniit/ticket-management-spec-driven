...
const BLANK_STEP: EscalationStepRequest = {
  waitTimeMinutes: 0,
  targetType: 'USER',
  targetId: '',
};
...
/**
 * Centered modal overlay for creating a new Escalation Policy.
 * - Name (required), Description (optional).
 * - Dynamic step list: starts with 1 blank step, enforces minimum 1.
 * - Client-side validation before submission.
 * - Displays server-side error messages inline.
 */
export default function AddEscalationPolicyModal({ isOpen, onClose }: Props) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [steps, setSteps] = useState<EscalationStepRequest[]>([{ ...BLANK_STEP }]);

  const [nameError, setNameError] = useState('');
  const [stepErrors, setStepErrors] = useState<StepErrors[]>([]);
  const [serverError, setServerError] = useState<string | null>(null);

  const { mutate: createPolicy, isPending } = useCreateEscalationPolicy();
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    createPolicy(payload, {
      ...
      onError: (err: any) => {
        setServerError(
          err?.response?.data?.message ?? 'Failed to create escalation policy. Please try again.',
        );
      },
    });
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-4xl mx-4 bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        {/* Scrollable body */}
        <form id="add-escalation-policy-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
          {/* Server error */}
          {serverError && (
          ...
          {/* Name */}
          <div>
            <label htmlFor="add-policy-name" className="block text-sm font-medium text-slate-700 mb-1.5">
          ...
          {/* Description */}
          <div>
            <label htmlFor="add-policy-description" className="block text-sm font-medium text-slate-700 mb-1.5">
              Description <span className="text-xs text-slate-400 font-normal">(optional)</span>
            </label>
            <textarea
              id="add-policy-description"
              rows={3}
              value={description}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl shrink-0">
          ...
          <button
            type="submit"
            form="add-escalation-policy-form"
            disabled={isPending}
